...
bool 平最大獲利多單()
{
    ...
    {
        isClosed = 多單平倉(OrderTicket(), OrderLots(), BID(), 0, clrNONE);
    }
...
void OnTick()
{
    ...
    if (讀取多單數量() == 0)
    {
        if (下單(TRADE_PAIR, OP_BUY, 單位手數, Ask, 1, 0, 0, StringConcatenate("MagicNumber:",MAGIC_NUMBER), MAGIC_NUMBER, 0, clrNONE))
        {
            更新價位();
        }
    }
